...
bool dgFoamVolumeToVTU::read(const dictionary& dict)
{
    ...
    if (hasOutputPoints)
    {
        ...
        {
            FatalIOErrorInFunction(dict)
                << "Expected 'outputPoints >= 2' for dgFoamVolumeToVTU, got "
    ...
    else if (hasLegacyN)
    {
        ...
        {
            FatalIOErrorInFunction(dict)
                << "Expected legacy 'n >= 1' for dgFoamVolumeToVTU, got " << n_
                << exit(FatalIOError);
        }
    }
    else
...
void dgFoamVolumeToVTU::initialize()
{
    ...
    {
        FatalIOErrorInFunction(thermoDict)
            << "Computed Cv = Cp - R <= 0 for dgFoamVolumeToVTU."
            << nl
            << "Cp = " << Cp_ << ", R = " << R_
            << exit(FatalIOError);
    }
...
const dgEquiSpacedTessellation& dgFoamVolumeToVTU::tessellation() const
{
    if (!tessellationPtr_)
    {
        FatalErrorInFunction
...
template<class Type>
Type dgFoamVolumeToVTU::evaluateField
(
...
void dgFoamVolumeToVTU::writePiece(const fileName& filePath) const
{
    ...
    forAll(meshRef.cells(), cellI)
    {
        ...
        const label collapsedRefDir =
          ...
          : -1;
        const List<dgEquiSpacedTessellation::VtkSubCell>& subCells =
            twoDMode_
          ? tessellationRef.collapsedVtkSubCells(type, collapsedRefDir)
          : tessellationRef.vtkSubCells(type);
        ...
        forAll(subCells, subCellI)
        {
            const dgEquiSpacedTessellation::VtkSubCell& subCell =
                subCells[subCellI];
            point subCellCenter(vector::zero);

            for (label pointI = 0; pointI < subCell.nPoints; ++pointI)
            {
                subCellCenter += localPoints[subCell.pointLabels[pointI]];
            }

            subCellCenter /= scalar(subCell.nPoints);

            const vector centerEta = mapXToEta(subCellCenter, cellVertices, type);
            List<scalar> centerBasis;
            tessellationRef.basisAt(type, centerEta, centerBasis);

            const scalar rhoValue = evaluateField(rho, cellI, centerBasis);
            const vector rhoUValue = evaluateField(rhoU, cellI, centerBasis);
            const scalar EValue = evaluateField(E, cellI, centerBasis);
    ...
    {
        FatalErrorInFunction
            << "Cell-data count mismatch while writing VTU: nCells=" << nCells
            << ", p=" << pCellValues.size()
            << ", T=" << TCellValues.size()
            << ", U=" << UCellValues.size()
            << exit(FatalError);
