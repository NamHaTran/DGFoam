...
#include "dgCompressiblePressureOutflowNonReflectiveBoundaryField.H"
...
defineTypeNameAndDebug(dgCompressiblePressureOutflowNonReflectiveBoundaryField, 0);
    ...
    subsonicMachThreshold_(0.99)
{
    const dictionary& coeffDict =
        dict.subDict("compressiblePressureOutflowNonReflectiveCoeff");
...
void dgCompressiblePressureOutflowNonReflectiveBoundaryField::updateGhostState
(
    const label,
    const label,
    const label,
...
) const
{
    ...
    const scalar boundaryEnergy =
        pressureVelocityToConservativeEnergy(pValue_, rhoMinusSafe, UMinus);

    EPlus = 2.0*boundaryEnergy - EMinus;
}
